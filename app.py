...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # title
        self.logo_label = customtkinter.CTkLabel(self, text="Farming Scripts", font=customtkinter.CTkFont(size=30, weight="bold"))
        self.logo_label.grid(row=0, column=0, padx=60, pady=25)

        # select script
        self.select_label = customtkinter.CTkLabel(self, text="Select Script", font=customtkinter.CTkFont(size=18))
        self.select_label.grid(row=1, column=0, padx=100, pady=5)

        self.selection_menu = customtkinter.CTkOptionMenu(self, values=["Nether Wart", "Sugar Cane", "Cactus"])
        self.selection_menu.grid(row=2, column=0, padx=100, pady=5)

        # select amount of farming rows
        self.select_label = customtkinter.CTkLabel(self, text="Amount of Rows", font=customtkinter.CTkFont(size=18))
        self.select_label.grid(row=3, column=0, padx=100, pady=5)

        self.farming_rows = customtkinter.CTkEntry(self, placeholder_text="number here")
        self.farming_rows.grid(row=4, column=0, padx=100, pady=5)
        
        # play pause keybind
        self.play_pause_label = customtkinter.CTkLabel(self, text="Play/Pause Keybind", font=customtkinter.CTkFont(size=18))
        self.play_pause_label.grid(row=5, column=0, padx=100, pady=5)

        self.play_pause_keybind = customtkinter.CTkEntry(self, placeholder_text="single key here")
        self.play_pause_keybind.grid(row=6, column=0, padx=100, pady=5)

        # run button

        self.run_button = customtkinter.CTkButton(self, text="Run Script", font=customtkinter.CTkFont(size=18), command=self.run_script)
